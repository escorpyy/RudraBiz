...
export default function UnitsTable({ rows }: { rows: UnitRow[] }) {
  ...
  const [contextMenuRow, setContextMenuRow] = useState<UnitRow | null>(null);
  ...
  async function handleDelete() {
    ...
    setDeleteError(null);
    try {
      const res = await fetch(`/api/product-units/${deleteTarget.id}`, { method: "DELETE" });
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-card">
              ...
              >
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center justify-end gap-1.5">
                    <Link href={`/master/units/${row.id}`} title="View" className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100 hover:text-slate-700">
                      <Eye size={16} />
                    </Link>
                    <Link href={`/master/units/${row.id}/edit`} title="Edit" className="rounded-md p-1.5 text-brand hover:bg-blue-50">
                      <Pencil size={16} />
                    </Link>
                    <button
                      title="Delete"
      ...
      <ConfirmDialog
        ...
        message="This permanently deletes the unit. Units currently used on a product can't be deleted until reassigned."
        error={deleteError}
        loading={deleting}
