...
export type ProductSubGroupFormInitial = {
  id: number;
...
export default function ProductSubGroupForm({ initial }: { initial?: ProductSubGroupFormInitial }) {
  ...
  useEffect(() => {
    fetch("/api/product-groups")
      .then((r) => r.json())
      .then((rows) => setProductGroups(rows.filter((g: ProductGroupOption) => g.isActive)))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!productGroupId || !code.trim() || !name.trim()) {
      setError("Product Group, Code, and Name are required.");
      return;
    }
    ...
    try {
      const res = await fetch(
        isEdit ? `/api/product-sub-groups/${initial!.id}` : "/api/product-sub-groups",
        {
      ...
      if (!res.ok) {
        ...
        throw new Error(body.error ?? "Failed to save product sub-group.");
      }

      router.push(isEdit ? `/master/product-sub-groups/${initial!.id}` : "/master/product-sub-groups");
      router.refresh();
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-xl border border-slate-200 bg-white p-6 shadow-card">
      <div>
        <h2 className="text-base font-semibold text-slate-900">Product Sub-Group Information</h2>
        <p className="mt-1 text-sm text-slate-500">
          {isEdit ? "Update details of this product sub-group." : "Provide details of the new product sub-group."}
        </p>
      </div>
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          <div>
            ...
            <p className="mb-2 text-xs text-slate-500">Unique within the selected group.</p>
            <input
              value={code}
          ...
          <div>
            ...
            <p className="mb-2 text-xs text-slate-500">Name of the sub-group.</p>
            <input
              value={name}
      ...
      <div className="flex justify-end gap-3">
        <button
          type="button"
          onClick={() => router.push(isEdit ? `/master/product-sub-groups/${initial!.id}` : "/master/product-sub-groups")}
