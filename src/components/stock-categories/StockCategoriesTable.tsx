...
export default function StockCategoriesTable({ rows }: { rows: StockCategoryRow[] }) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<StockCategoryRow | null>(null);
  ...
  const { runBulkPatch, runBulkDelete, running } = useBulkApi("/api/stock-categories");
  const [contextMenu, setContextMenu] = useState<ContextMenuPosition>(null);
  const [contextMenuRow, setContextMenuRow] = useState<StockCategoryRow | null>(null);
  ...
  async function handleDelete() {
    ...
    setDeleteError(null);
    try {
      const res = await fetch(`/api/stock-categories/${deleteTarget.id}`, { method: "DELETE" });
      const body = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(body.error ?? "Failed to delete stock category.");
  ...
  function openContextMenu(e: React.MouseEvent, row: StockCategoryRow, index: number) {
    e.preventDefault();
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-card">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {pageRows.map((row, i) => (
              ...
              >
                ...
                <td className="px-3 py-3.5">
                  <span className="font-medium text-brand">{row.productCount}</span>
                </td>
                <td className="px-3 py-3.5">
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center justify-end gap-1.5">
                    <Link href={`/master/stock-categories/${row.id}`} title="View" className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100 hover:text-slate-700">
                      <Eye size={16} />
                    </Link>
                    <Link href={`/master/stock-categories/${row.id}/edit`} title="Edit" className="rounded-md p-1.5 text-brand hover:bg-blue-50">
                      <Pencil size={16} />
                    </Link>
                    <button
                      title="Delete"
            ...
            {pageRows.length === 0 && (
              <tr>
                <td colSpan={selection.selectionMode ? 7 : 6} className="px-5 py-10 text-center text-sm text-slate-400">
                  No stock categories match your filters.
                </td>
              </tr>
            )}
          </tbody>
      ...
      <ConfirmDialog
        ...
        message="This permanently deletes the stock category and unlinks it from any products using it. This can't be undone."
        error={deleteError}
        loading={deleting}
      ...
      <ConfirmDialog
        ...
        title={`Delete ${selection.count} stock categor${selection.count === 1 ? "y" : "ies"}?`}
        message="Each one is deleted individually — any still linked to products will unlink rather than block the rest. This can't be undone."
        loading={running}
