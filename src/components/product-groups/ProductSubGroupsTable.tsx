...
export type ProductSubGroupRow = {
  id: number;
  code: string;
  name: string;
  groupName: string;
...
export default function ProductSubGroupsTable({ rows }: { rows: ProductSubGroupRow[] }) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<ProductSubGroupRow | null>(null);
  ...
  const { runBulkPatch, runBulkDelete, running } = useBulkApi("/api/product-sub-groups");
  const [contextMenu, setContextMenu] = useState<ContextMenuPosition>(null);
  const [contextMenuRow, setContextMenuRow] = useState<ProductSubGroupRow | null>(null);
  ...
  async function handleDelete() {
    ...
    setDeleteError(null);
    try {
      const res = await fetch(`/api/product-sub-groups/${deleteTarget.id}`, { method: "DELETE" });
      const body = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(body.error ?? "Failed to delete product sub-group.");
  ...
  function openContextMenu(e: React.MouseEvent, row: ProductSubGroupRow, index: number) {
    e.preventDefault();
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-card">
      ...
      <div className="flex gap-6 border-b border-slate-200 px-5 pt-4">
        <Link href="/master/product-groups" className="pb-3 text-sm font-medium text-slate-500 hover:text-slate-700">
          Groups
        </Link>
        <button className="border-b-2 border-brand pb-3 text-sm font-medium text-brand">Sub-Groups</button>
        <Link href="/master/products" className="pb-3 text-sm font-medium text-slate-500 hover:text-slate-700">
          Products
        </Link>
      </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {pageRows.map((row, i) => (
              ...
              >
                ...
                <td className={selection.selectionMode ? "px-3 py-3.5 text-slate-500" : "px-5 py-3.5 text-slate-500"}>{(currentPage - 1) * pageSize + i + 1}</td>
                <td className="px-3 py-3.5 font-medium text-slate-900">{row.code}</td>
                <td className="px-3 py-3.5 text-slate-800">{row.name}</td>
                <td className="px-3 py-3.5 text-slate-500">{row.groupName}</td>
                <td className="px-3 py-3.5">
                  <span className="font-medium text-brand">{row.productsCount}</span>
                </td>
                <td className="px-3 py-3.5">
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center justify-end gap-1.5">
                    <Link href={`/master/product-sub-groups/${row.id}`} title="View" className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100 hover:text-slate-700">
                      <Eye size={16} />
                    </Link>
                    <Link href={`/master/product-sub-groups/${row.id}/edit`} title="Edit" className="rounded-md p-1.5 text-brand hover:bg-blue-50">
                      <Pencil size={16} />
                    </Link>
                    <button
                      title="Delete"
            ...
            {pageRows.length === 0 && (
              <tr>
                <td colSpan={selection.selectionMode ? 8 : 7} className="px-5 py-10 text-center text-sm text-slate-400">
      ...
      <ConfirmDialog
        ...
        message="This permanently deletes the product sub-group. This can't be undone."
        error={deleteError}
        loading={deleting}
      ...
      <ConfirmDialog
        ...
        title={`Delete ${selection.count} sub-group${selection.count === 1 ? "" : "s"}?`}
        message="Each one is deleted individually — any with existing products will be reported as failed rather than blocking the rest. This can't be undone."
        loading={running}
