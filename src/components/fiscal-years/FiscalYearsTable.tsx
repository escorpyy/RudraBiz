...
export default function FiscalYearsTable({ rows }: { rows: FiscalYearRow[] }) {
  ...
  const filtered = useMemo(() => {
    return rows.filter((row) => {
      const matchesSearch = search.trim() === "" || row.code.toLowerCase().includes(search.toLowerCase());
      const matchesStatus = statusFilter === "ALL" || row.status === statusFilter;
      const matchesPeriod =
        periodFilter === "ALL" || (periodFilter === "CLOSED" ? row.isClosed : !row.isClosed);
  ...
  const { runBulkPatch, runBulkDelete, running } = useBulkApi("/api/fiscal-years");
  const [contextMenu, setContextMenu] = useState<ContextMenuPosition>(null);
  const [contextMenuRow, setContextMenuRow] = useState<FiscalYearRow | null>(null);
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(body.error ?? "Failed to delete fiscal year.");
  ...
  function openContextMenu(e: React.MouseEvent, row: FiscalYearRow, index: number) {
    e.preventDefault();
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-card">
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-y border-slate-200 text-xs font-medium uppercase tracking-wide text-slate-500">
              ...
              <th className="px-3 py-3 font-medium">Locked</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {pageRows.map((row, i) => (
              ...
              >
                ...
                <td className="px-3 py-3.5">
                  {row.isOpeningBalanceLocked ? (
                    <span title="Opening balances locked" className="inline-flex items-center text-amber-600">
                      <Lock size={15} />
                    </span>
                  ) : (
                    <span className="text-slate-400">—</span>
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center justify-end gap-1.5">
                    <Link href={`/master/fiscal-years/${row.id}`} title="View" className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100 hover:text-slate-700">
                      <Eye size={16} />
                    </Link>
                    <Link href={`/master/fiscal-years/${row.id}/edit`} title="Edit" className="rounded-md p-1.5 text-brand hover:bg-blue-50">
                      <Pencil size={16} />
                    </Link>
                    <button
                      title="Delete"
            ...
            {pageRows.length === 0 && (
              <tr>
                <td colSpan={selection.selectionMode ? 9 : 8} className="px-5 py-10 text-center text-sm text-slate-400">
                  No fiscal years match your filters.
                </td>
              </tr>
            )}
          </tbody>
      ...
      <ConfirmDialog
        ...
        message="This permanently deletes the fiscal year. This can't be undone."
        error={deleteError}
        loading={deleting}
      ...
      <ConfirmDialog
        ...
        title={`Delete ${selection.count} fiscal year${selection.count === 1 ? "" : "s"}?`}
        message="This can't be undone. Any year with opening balances or that's set as current will be skipped."
        loading={running}
