...
export default function AgentsTable({ rows }: { rows: AgentRow[] }) {
  ...
  const { runBulkPatch, runBulkDelete, running } = useBulkApi("/api/agents");
  const [contextMenu, setContextMenu] = useState<ContextMenuPosition>(null);
  const [contextMenuRow, setContextMenuRow] = useState<AgentRow | null>(null);
  ...
  function openContextMenu(e: React.MouseEvent, row: AgentRow, index: number) {
    e.preventDefault();
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-card">
          ...
          <tbody className="divide-y divide-slate-100">
            {pageRows.map((row, i) => (
              ...
              >
                ...
                <td className="px-3 py-3.5 text-slate-500">{row.phone || "—"}</td>
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center justify-end gap-1.5">
                    ...
                    <Link
                      href={`/master/agents/${row.id}/edit`}
                      title="Edit"
      ...
      <ConfirmDialog
        ...
        message="This permanently deletes the agent and unlinks it from any ledgers, parties, or vouchers it was tagged on. This can't be undone."
        error={deleteError}
        loading={deleting}
      ...
      <ConfirmDialog
        ...
        title={`Delete ${selection.count} agent${selection.count === 1 ? "" : "s"}?`}
        message="Each one is deleted individually and unlinked from any ledgers, parties, or vouchers it was tagged on. This can't be undone."
        loading={running}
