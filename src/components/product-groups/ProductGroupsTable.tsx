...
export default function ProductGroupsTable({ rows }: { rows: ProductGroupRow[] }) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<ProductGroupRow | null>(null);
  ...
  const { runBulkPatch, runBulkDelete, running } = useBulkApi("/api/product-groups");
  const [contextMenu, setContextMenu] = useState<ContextMenuPosition>(null);
  const [contextMenuRow, setContextMenuRow] = useState<ProductGroupRow | null>(null);
  ...
  async function handleDelete() {
    ...
    setDeleteError(null);
    try {
      const res = await fetch(`/api/product-groups/${deleteTarget.id}`, { method: "DELETE" });
      const body = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(body.error ?? "Failed to delete product group.");
  ...
  function openContextMenu(e: React.MouseEvent, row: ProductGroupRow, index: number) {
    e.preventDefault();
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-card">
      ...
      <div className="flex gap-6 border-b border-slate-200 px-5 pt-4">
        ...
        <Link href="/master/product-sub-groups" className="pb-3 text-sm font-medium text-slate-500 hover:text-slate-700">
          Sub-Groups
        </Link>
              ...
              >
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center justify-end gap-1.5">
                    <Link href={`/master/product-groups/${row.id}`} title="View" className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100 hover:text-slate-700">
                      <Eye size={16} />
                    </Link>
                    <Link href={`/master/product-groups/${row.id}/edit`} title="Edit" className="rounded-md p-1.5 text-brand hover:bg-blue-50">
                      <Pencil size={16} />
                    </Link>
                    <button
                      title="Delete"
      ...
      <ConfirmDialog
        ...
        message="This permanently deletes the product group. This can't be undone."
        error={deleteError}
        loading={deleting}
      ...
      <ConfirmDialog
        ...
        title={`Delete ${selection.count} product group${selection.count === 1 ? "" : "s"}?`}
        message="Each one is deleted individually — any with existing sub-groups will be reported as failed rather than blocking the rest. This can't be undone."
        loading={running}
