...
export default async function EditCashBankVoucherPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const { companyId, fiscalYearId } = await getCompanyContext();

  const [voucher, branches, fiscalYear] = await Promise.all([
    companyId
      ? prisma.cashBankVoucher.findFirst({
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-1.5 text-sm">
            ...
            <Link href={`/transactions/cash-bank-voucher/${voucher.id}`} className="text-brand hover:underline">
              {voucher.voucherNumber}
            </Link>
            <span className="text-slate-400">&gt;</span>
            <span className="text-slate-500">Edit</span>
          </div>
          <h1 className="mt-1 text-2xl font-bold text-slate-900">Edit Cash / Bank Voucher</h1>
        </div>
        <Link
          href={`/transactions/cash-bank-voucher/${voucher.id}`}
        ...
        <CashBankVoucherForm
          initial={{
            ...
            lines: voucher.lines.map((line) => ({
              ...
              instrumentType: line.instrumentType ?? "",
              chequeNumber: line.chequeNumber ?? "",
              chequeDate: line.chequeDate ? line.chequeDate.toISOString().slice(0, 10) : "",
              chequeBankName: line.chequeBankName ?? "",
              chequeBankBranch: line.chequeBankBranch ?? "",
            })),
          }}
