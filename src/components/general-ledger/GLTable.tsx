...
import NormalBalanceBadge from "./NormalBalanceBadge";
import GLTypeBadge from "./GLTypeBadge";
import StatusBadge from "@/components/account-groups/StatusBadge";
import { GL_TYPE_LIST, GL_TYPES, type GLType, type NormalBalance } from "@/lib/constants";
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-y border-slate-200 text-xs font-medium uppercase tracking-wide text-slate-500">
              ...
              <th className="px-3 py-3 font-medium">Sub-Group</th>
              <th className="px-3 py-3 font-medium">Normal Balance</th>
              ...
              <tr key={row.id} className="hover:bg-slate-50/60">
                ...
                <td className="px-3 py-3.5 text-slate-800">
                  <div className="flex items-center gap-2">
                    {row.isCashOrBank && <Landmark size={14} className="text-emerald-500" />}
                    {row.name}
                  </div>
                </td>
                <td className="px-3 py-3.5 text-slate-500">{row.accountSubGroupName}</td>
                <td className="px-3 py-3.5">
                  <NormalBalanceBadge value={row.normalBalance} />
                </td>
                <td className="px-3 py-3.5">
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center justify-end gap-1.5">
                    <button title="View" className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100 hover:text-slate-700">
                      <Eye size={16} />
                    </button>
                    <button title="Edit" className="rounded-md p-1.5 text-brand hover:bg-blue-50">
                      <Pencil size={16} />
                    </button>
                    <button title="Delete" className="rounded-md p-1.5 text-rose-500 hover:bg-rose-50">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </td>
