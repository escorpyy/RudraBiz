...
import { ChevronDown, Ruler, Save } from "lucide-react";
...
export default function UnitForm({ initial }: { initial?: UnitFormInitial }) {
  const router = useRouter();
  const isEdit = Boolean(initial);
  const [code, setCode] = useState(initial?.code ?? "");
  const [name, setName] = useState(initial?.name ?? "");
  const [decimalPlaces, setDecimalPlaces] = useState(String(initial?.decimalPlaces ?? 0));
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(isEdit ? `/api/product-units/${initial!.id}` : "/api/product-units", {
      ...
      if (!res.ok) {
        ...
        throw new Error(body.error ?? "Failed to save unit.");
      }

      router.push(isEdit ? `/master/units/${initial!.id}` : "/master/units");
      router.refresh();
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-xl border border-slate-200 bg-white p-6 shadow-card">
      <div>
        <h2 className="text-base font-semibold text-slate-900">Unit Information</h2>
        <p className="mt-1 text-sm text-slate-500">
          {isEdit ? "Update details of this unit." : "Provide details of the new unit."}
        </p>
      </div>
      ...
      <div className="space-y-5">
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          <div>
            ...
            <p className="mb-2 text-xs text-slate-500">Unique code, e.g. PCS, KG, BOX.</p>
            <div className="relative">
              <Ruler size={16} className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-blue-500" />
              <input
                value={code}
          ...
          <div>
            ...
            <p className="mb-2 text-xs text-slate-500">Full name of the unit.</p>
            <input
              value={name}
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          <div>
            <label className="text-sm font-medium text-slate-800">Decimal Places</label>
            <p className="mb-2 text-xs text-slate-500">0 for whole-count units like PCS; 2–3 for KG/LTR.</p>
            <input
              type="number"
              min={0}
              max={6}
          ...
          <div>
            ...
            <p className="mb-2 text-xs text-slate-500">Set active to make this unit selectable.</p>
            <div className="relative">
              <span
      ...
      <div className="flex justify-end gap-3">
        <button
          type="button"
          onClick={() => router.push(isEdit ? `/master/units/${initial!.id}` : "/master/units")}
        ...
        >
          <Save size={16} />
          {submitting ? "Saving..." : isEdit ? "Update Unit" : "Save Unit"}
        </button>
      </div>
    </form>
  );
