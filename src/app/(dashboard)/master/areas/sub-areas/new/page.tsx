...
// SubAreaForm reads useSearchParams (to prefill areaId), which requires
// this route not be statically prerendered.
export const dynamic = "force-dynamic";

export default function NewSubAreaPage() {
  return (
    <div>
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Add New Sub-Area</h1>
          <div className="mt-1 flex items-center gap-1.5 text-sm">
            <Link href="/master/areas/sub-areas" className="text-brand hover:underline">
              Sub-Areas
            </Link>
            <span className="text-slate-400">&gt;</span>
            <span className="text-slate-500">Add New Sub-Area</span>
          </div>
        </div>
        <Link
