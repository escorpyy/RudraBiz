...
export default function SubGroupsTable({
  ...
  const { runBulkPatch, runBulkDelete, running } = useBulkApi("/api/account-sub-groups");
  const [contextMenu, setContextMenu] = useState<ContextMenuPosition>(null);
  const [contextMenuRow, setContextMenuRow] = useState<SubGroupRow | null>(null);
  ...
  function openContextMenu(e: React.MouseEvent, row: SubGroupRow, index: number) {
    e.preventDefault();
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-card">
      ...
      <ConfirmDialog
        ...
        message="Each one is deleted individually — any with existing ledgers will be reported as failed rather than blocking the rest. This can't be undone."
        loading={running}
