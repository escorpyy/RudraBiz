...
import { PRODUCT_TYPE_LIST, PRODUCT_TYPES, type ProductType } from "@/lib/constants";
...
export default function ProductsTable({ rows }: { rows: ProductRow[] }) {
  ...
  const filtered = useMemo(() => {
    return rows.filter((row) => {
      ...
      const matchesStatus = statusFilter === "ALL" || (statusFilter === "ACTIVE") === row.isActive;
  ...
  const { runBulkPatch, runBulkDelete, running } = useBulkApi("/api/products");
  const [contextMenu, setContextMenu] = useState<ContextMenuPosition>(null);
  const [contextMenuRow, setContextMenuRow] = useState<ProductRow | null>(null);
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(body.error ?? "Failed to delete product.");
  ...
  function openContextMenu(e: React.MouseEvent, row: ProductRow, index: number) {
    e.preventDefault();
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-card">
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-y border-slate-200 text-xs font-medium uppercase tracking-wide text-slate-500">
              ...
              <th className="px-3 py-3 font-medium">Group / Sub-Group</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {pageRows.map((row, i) => (
              ...
              >
                ...
                <td className="px-3 py-3.5 text-slate-800">{row.description}</td>
                <td className="px-3 py-3.5">
                  <ProductTypeBadge type={row.type} />
                </td>
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center justify-end gap-1.5">
                    ...
                    <Link
                      href={`/master/products/${row.id}/edit`}
                      title="Edit"
      ...
      <ConfirmDialog
        ...
        message="This permanently deletes the product and its detail record. This can't be undone."
        error={deleteError}
        loading={deleting}
      ...
      <ConfirmDialog
        ...
        message="Each one is deleted individually — any still used as a bundle component will be reported as failed rather than blocking the rest. This can't be undone."
        loading={running}
