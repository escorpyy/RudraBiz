...
export default function LocationsTable({ rows }: { rows: LocationRow[] }) {
  ...
  const { runBulkPatch, runBulkDelete, running } = useBulkApi("/api/locations");
  const [contextMenu, setContextMenu] = useState<ContextMenuPosition>(null);
  const [contextMenuRow, setContextMenuRow] = useState<LocationRow | null>(null);
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(body.error ?? "Failed to delete location.");
  ...
  function openContextMenu(e: React.MouseEvent, row: LocationRow, index: number) {
    e.preventDefault();
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-card">
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-y border-slate-200 text-xs font-medium uppercase tracking-wide text-slate-500">
              ...
              <th className="px-3 py-3 font-medium">Sub-Locations</th>
            ...
            {pageRows.map((row, i) => (
              ...
              >
                ...
                <td className="px-3 py-3.5">
                  <span className="font-medium text-brand">{row.childrenCount}</span>
                </td>
                <td className="px-3 py-3.5">
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center justify-end gap-1.5">
                    <Link href={`/master/locations/${row.id}`} title="View" className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100 hover:text-slate-700">
                      <Eye size={16} />
                    </Link>
                    <Link href={`/master/locations/${row.id}/edit`} title="Edit" className="rounded-md p-1.5 text-brand hover:bg-blue-50">
                      <Pencil size={16} />
                    </Link>
                    <button
                      title="Delete"
      ...
      <ConfirmDialog
        ...
        message="This permanently deletes the location. Locations with sub-locations can't be deleted until those are reassigned."
        error={deleteError}
        loading={deleting}
      ...
      <ConfirmDialog
        ...
        title={`Delete ${selection.count} location${selection.count === 1 ? "" : "s"}?`}
        message="Each one is deleted individually — any with existing sub-locations will be reported as failed rather than blocking the rest. This can't be undone."
        loading={running}
