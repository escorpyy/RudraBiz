...
export default function SubAreasTable({
  ...
  const [deleteTarget, setDeleteTarget] = useState<SubAreaRow | null>(null);
  ...
  const filtered = useMemo(() => {
    return rows.filter((row) => {
      ...
      const matchesArea = areaFilter === "ALL" || String(row.areaId) === areaFilter;
  ...
  async function handleDelete() {
    ...
    setDeleteError(null);
    try {
      const res = await fetch(`/api/sub-areas/${deleteTarget.id}`, { method: "DELETE" });
      const body = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(body.error ?? "Failed to delete sub-area.");
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-card">
      ...
      <div className="flex gap-6 border-b border-slate-200 px-5 pt-4">
        ...
        <button className="border-b-2 border-brand pb-3 text-sm font-medium text-brand">Sub-Areas</button>
      </div>
              ...
              <tr key={row.id} className="hover:bg-slate-50/60">
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center justify-end gap-1.5">
                    <Link
                      href={`/master/areas/sub-areas/${row.id}`}
                      title="View"
                    ...
                    <Link
                      href={`/master/areas/sub-areas/${row.id}/edit`}
                      title="Edit"
      ...
      <ConfirmDialog
        ...
        message="This permanently deletes the sub-area. This can't be undone."
        error={deleteError}
        loading={deleting}
