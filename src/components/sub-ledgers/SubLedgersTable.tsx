...
export default function SubLedgersTable({
  ...
  const { runBulkPatch, runBulkDelete, running } = useBulkApi("/api/sub-ledgers");
  const [contextMenu, setContextMenu] = useState<ContextMenuPosition>(null);
  const [contextMenuRow, setContextMenuRow] = useState<SubLedgerRow | null>(null);
  ...
  function openContextMenu(e: React.MouseEvent, row: SubLedgerRow, index: number) {
    e.preventDefault();
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-card">
      ...
      <ConfirmDialog
        ...
        title={`Delete ${selection.count} sub-ledger${selection.count === 1 ? "" : "s"}?`}
