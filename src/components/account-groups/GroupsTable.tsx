...
export default function GroupsTable({ rows }: { rows: AccountGroupRow[] }) {
  ...
  const { runBulkPatch, runBulkDelete, running } = useBulkApi("/api/account-groups");
  const [contextMenu, setContextMenu] = useState<ContextMenuPosition>(null);
  const [contextMenuRow, setContextMenuRow] = useState<AccountGroupRow | null>(null);
  ...
  function openContextMenu(e: React.MouseEvent, row: AccountGroupRow, index: number) {
    e.preventDefault();
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-card">
      ...
      <ConfirmDialog
        ...
        title={`Delete ${selection.count} account group${selection.count === 1 ? "" : "s"}?`}
